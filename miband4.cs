...
namespace MiBand_Heartrate_2.Devices
{
    public class MiBand4_Device : Device
    {
        ...
        public MiBand4_Device(DeviceInformation d, string key)
        {
            Name = d.Name;
        ...
        /* Mi Band 4 - Auth
         * Before using Mi Band 4, an auth is required. Authenticate following this steps :
         *  1. Enable notification on authentification characteristic
         *  2. Ask for authentication
         *  3. Get random number from characteristic notification
         *  4. Encrypt number using AES ECB mode without padding and with auth key
         *  5. Send encrypted random number to auth characteristic
         */
                ...
                if (service.Status == GattCommunicationStatus.Success && service.Services.Count > 0)
                {
                    ...
                    if (characteristic.Status == GattCommunicationStatus.Success && characteristic.Characteristics.Count > 0)
                    {
                        ...
                        if (notify == GattCommunicationStatus.Success)
                        {
                            ...
                            BLE.Write(characteristic.Characteristics[0], new byte[] { 0x02, 0x00 });
                        }
                    }
                }
            });
        ...
        void OnAuthenticateNotify(GattCharacteristic sender, GattValueChangedEventArgs args)
        {
            ...
            using (DataReader reader = DataReader.FromBuffer(args.CharacteristicValue))
            {
                ...
                if (headers[1] == 0x01)
                {
                    ...
                    {
                        StartPr.Log("Authentication failed (1)");
                    }
                }
                ...
                else if (headers[1] == 0x03)
                {
                    ...
                    {
                        StartPr.Log("Authentication failed (3)");
                    }
                }
            }
        }
        ...
        public override void StartHeartrateMonitor(bool continuous = false)
        {
            ...
            var task = Task.Run(async () =>
            {
                ...
                // });
                StartPr.Log("Hearth Monitor started: true");
            });
        }
        ...
        public override void StopHeartrateMonitor()
        {
            ...
            StartPr.Log("Hearth Monitor started: false");
